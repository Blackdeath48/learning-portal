...
- **Secure authentication** – simple email + password login backed by JWTs; the first registered account is promoted to administrator.
- **Learner dashboard** – monitor completion, scores, time spent, and recent statements with executive-ready visualisations.
- **Course catalog** – discover available programmes with rich metadata and filters.
- **Admin workspace** – administrators can create, edit, and delete courses and lessons, manage learner accounts, review analytics, and export CSV/PDF compliance reports.
...
2. **Configure environment variables**
   ...
   Copy the example file and supply your Neon connection string and JWT secret:

   ```bash
   cp .env.example .env
   ```

   Update `DATABASE_URL` to match your Neon project (keep `sslmode=require`). Set `JWT_SECRET` to a long, random value used to sign access tokens.

   > The first user who registers through `/register` is automatically granted the `ADMIN` role to bootstrap the platform. Subsequent registrations create standard learners; administrators can add or promote users from the admin workspace.
